...
void setup(void)
{
  ...
  Serial.begin(115200);
  Serial.println(F("Adafruit Bluefruit Heart Rate Monitor Temperature Example"));
  Serial.println(F("---------------------------------------------------"));
  ...
  attachInterrupt(28, buttonPressed, CHANGE);
  ...
  Serial.println(F("Setting device name to 'Adafruit Bluefruit LE': "));
  
  // Keeping it simple. Had issues with name not taking
  if (! ble.sendCommandCheckOK(F("AT+GAPDEVNAME=Adafruit Bluefruit LE")) ) {
...
void loop(void)
{
  ...
  int convertedMag = (int) (mag*(255/30));
  Serial.println(mag);
  Serial.println(convertedMag);
  Serial.println(x);
  Serial.println(y);
  Serial.println(z);
  ...
  ble.print( F(",00-") );
  int sendTemp = (int) millis();

  
  ble.println(convertedMag, HEX);
